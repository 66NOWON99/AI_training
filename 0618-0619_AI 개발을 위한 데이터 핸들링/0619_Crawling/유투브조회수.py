...
#화면 전체를 가져와서 크기정보로 스크롤을 하기 위해서 body 태그 객체를 가져온다
#find_elements_by_tag_name - 태그를 갖고오고자 할 때
body = driver.find_element_by_tag_name("body")
# i=1
#반복적으로 진행한다; 언제 끝날지 몰라서 무한루프로 진행한다

def scroll_screet(cnt):
    i=0
    while True:
        #현재 화면의 길이를 알아온다.
        last_height = driver.execute_script(
            "return document.documentElement.scrollHeight")#javascript 에서 화면 크기를 알아오는 method
        # print(last_height)
        ...
        #Keys - 키보드의 값들을 말함. 약속되어 있는 상수값. enter key: RETURN
        i = i+1
...
divList = bs.findAll("div", {"id":"dismissable","class":"style-scope ytd-grid-video-renderer"})
for video in divList:
    title = video.find("a",{"id":"video-title"})
    viewcnt = video.find("span",{"class":"style-scope ytd-grid-video-renderer"})
    if title != None:
