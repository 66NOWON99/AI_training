...
def index(request, content_topic_idx):
    #filter함수 : where 컬럼명=값 에 해당한다.
    topic_model = models.TopicModel.objects.filter(topic_idx=content_topic_idx)
    # 토론방에 해당하는 데이터를 가져온다.
    content_list = models.ContentModel.objects.filter(content_topic_idx=content_topic_idx).order_by('content_idx')

    data_dic = {
        'topic_name': topic_model,
