...
def index(request) :
    # all : select 문
    # order_by : order by 문. -를 붙히면 내림차순 정렬
    topic_list = models.TopicModel.objects.all().order_by('-topic_idx')

    # test = models.ContentModel.objects.all().values('content_topic_idx').annotate(a1=Count('content_text'))

    # test = models.ContentModel.objects.all().values().annotate(cnt=Count('content_text'))
    #
    # for obj in test:
    #     print(obj)
